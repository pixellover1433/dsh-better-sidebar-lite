...
export function GitStatusView({ result, root, rpc, t, onChanged, onActionError, onDiscard, onDiscardAll }) {
    ...
    return (_jsxs(_Fragment, { children: [discardable && (_jsx("div", { className: styles.discardAllRow, children: _jsx("button", { type: "button", className: styles.discardAllButton, onClick: onDiscardAll, children: t('discardAll') }) })), _jsxs("div", { className: styles.statusSections, children: [result.staged.length > 0 && (_jsx(Section, { title: t('staged'), entries: result.staged, t: t, onStage: onStage, onUnstage: onUnstage, onDiscard: onDiscard, sectionAction: {
                            label: t('unstageAll'),
                            onClick: () => { void unstagePaths(result.staged.map(e => e.path)); },
                        } })), result.conflicted.length > 0 && (_jsx(Section, { title: t('conflicts'), entries: result.conflicted, t: t, onStage: onStage, onUnstage: onUnstage, onDiscard: onDiscard, conflictStyles: true })), result.unstaged.length > 0 && (_jsx(Section, { title: t('changes'), entries: result.unstaged, t: t, onStage: onStage, onUnstage: onUnstage, onDiscard: onDiscard, sectionAction: {
                            label: t('stageAll'),
                            onClick: () => { void stagePaths(result.unstaged.map(e => e.path)); },
                        } })), result.untracked.length > 0 && (_jsx(Section, { title: t('untracked'), entries: result.untracked, t: t, onStage: onStage, onUnstage: onUnstage, onDiscard: onDiscard, sectionAction: {
                            label: t('stageAll'),
