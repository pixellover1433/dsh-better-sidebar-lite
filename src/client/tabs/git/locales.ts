/**
 * `betterSidebar.git` namespace dictionaries. The zh dictionary is the key-set
 * source of truth; en is key-identical. client-core registers this namespace
 * via ctx.locale.register; the LocaleNamespaceMap merge below makes the typed
 * translate (TranslateNS<'betterSidebar.git'>) reject typos at compile time.
 */
import type {} from '@deepseek-ai/dsh-client-locale/client'
import type {} from '@deepseek-ai/dsh-client-ui-slots'

/** Dictionary namespace owned by the git tab. */
export const NS = 'betterSidebar.git' as const

/** Simplified-Chinese dictionary (source of truth for the key set). */
export const zh = {
  tabLabel: 'Git',
  refresh: '刷新',
  staged: '已暂存',
  conflicts: '冲突',
  changes: '更改',
  untracked: '未跟踪',
  stage: '暂存',
  unstage: '取消暂存',
  stageAll: '全部暂存',
  unstageAll: '全部取消暂存',
  loadMore: '加载更多',
  notARepo: '这不是 Git 仓库',
  notARepoHint: '当前工作区不是 Git 仓库。可以使用其他路径或重新尝试。',
  gitMissing: '未找到 Git',
  gitMissingHint: '未检测到 Git 可执行文件。请确认已安装 Git 后重试。',
  errorRetry: '重试',
  branch: '分支',
  emptyStatus: '没有更改，工作区是干净的',
  emptyLog: '还没有提交',
  emptyCommitFiles: '该提交没有文件变更',
  commitDetailTitle: '查看提交详情',
  back: '返回',
  commit: '提交',
  commitMessage: '提交信息',
  commitPlaceholder: '输入提交信息…',
  commitAll: '包含所有更改（自动暂存）',
  commitStagedHint: '无已暂存更改',
  discard: '丢弃',
  discardConfirm: '确定要丢弃文件 “{path}” 的所有更改吗？此操作不可撤销。',
  discardAll: '全部丢弃',
  discardAllConfirm: '确定要丢弃所有未暂存的更改和未跟踪的文件吗？此操作不可撤销。',
  loading: '加载中…',
  noWorkspace: '未打开工作区',
  noWorkspaceHint: 'Git 标签页需要一个工作区目录。请打开或选择一个工作区。',
} as const

/** English dictionary, key-identical to the zh source of truth. */
export const en: Record<GitKey, string> = {
  tabLabel: 'Git',
  refresh: 'Refresh',
  staged: 'Staged',
  conflicts: 'Conflicts',
  changes: 'Changes',
  untracked: 'Untracked',
  stage: 'Stage',
  unstage: 'Unstage',
  stageAll: 'Stage all',
  unstageAll: 'Unstage all',
  loadMore: 'Load more',
  notARepo: 'Not a Git repository',
  notARepoHint: 'The current workspace is not a Git repository. Try a different path or refresh.',
  gitMissing: 'Git not found',
  gitMissingHint: 'No Git executable detected. Make sure Git is installed, then retry.',
  errorRetry: 'Retry',
  branch: 'Branch',
  emptyStatus: 'No changes — the working tree is clean',
  emptyLog: 'No commits yet',
  emptyCommitFiles: 'No files changed in this commit',
  commitDetailTitle: 'View commit details',
  back: 'Back',
  commit: 'Commit',
  commitMessage: 'Commit message',
  commitPlaceholder: 'Enter a commit message…',
  commitAll: 'Include all changes (stage everything)',
  commitStagedHint: 'Nothing staged to commit',
  discard: 'Discard',
  discardConfirm: 'Discard all changes to "{path}"? This cannot be undone.',
  discardAll: 'Discard all',
  discardAllConfirm: 'Discard all uncommitted changes and untracked files? This cannot be undone.',
  loading: 'Loading…',
  noWorkspace: 'No workspace open',
  noWorkspaceHint: 'The Git tab needs a workspace directory. Open or select one to continue.',
}

/** Key domain of the git namespace. */
export type GitKey = keyof typeof zh

declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface LocaleNamespaceMap {
    /** Git tab copy. */
    'betterSidebar.git': GitKey
  }
}