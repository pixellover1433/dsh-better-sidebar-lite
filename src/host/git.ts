...
import { GitRunner, type RunGitResult } from './git-runner.ts'
import { parsePorcelainV1Z } from './git-status-parser.ts'
...
/** Result of the rev-parse head probe. */
...
function mapRunnerError(res: Extract<RunGitResult, { ok: false }>, root: string): SidebarError {
  switch (res.kind) {
    ...
    case 'not-a-repo': return { code: 'not-a-repo', message: res.message, path: root } as SidebarError
    case 'git-failed': return { code: 'git-failed', message: res.message, stderrTail: res.stderrTail ?? '' } as SidebarError
  }
}
...
export class GitService {
  constructor(
    ...
    private readonly opts: GitServiceOptions,
  ) {}
   ...
   */
  private async probe(cwd: string, signal?: AbortSignal): Promise<HeadResult | SidebarError> {
    const inside = await this.runner.run(['rev-parse', '--is-inside-work-tree'], cwd, signal)
    if (!inside.ok) return mapRunnerError(inside, cwd)
    if (inside.stdout.toString('utf8').trim() !== 'true') {
      return { code: 'not-a-repo', message: 'not a git repository', path: cwd } as SidebarError
    }
    const head = await this.runner.run(['rev-parse', '--abbrev-ref', 'HEAD'], cwd, signal)
    if (!head.ok) return mapRunnerError(head, cwd)
  ...
  status(request: GitStatusRequest, signal?: AbortSignal): Promise<SidebarResult<GitStatusResult>> {
    return this.withProbe(request.path, signal, async (head) => {
      ...
      const res = await this.runner.run(
        ['status', '--porcelain=v1', '-z', '--untracked-files=' + mode],
        request.path,
        signal,
      )
  ...
  log(request: GitLogRequest, signal?: AbortSignal): Promise<SidebarResult<GitLogResult>> {
    return this.withProbe(request.path, signal, async (head) => {
      ...
      const res = await this.runner.run(
        ['log', '-n', String(cap + 1), '--format=%H%x1f%h%x1f%an%x1f%ae%x1f%aI%x1f%s%x1e'],
        request.path,
        signal,
      )
      if (!res.ok) return { ok: false, error: mapRunnerError(res, request.path) }
  ...
  commitDetail(request: GitCommitDetailRequest, signal?: AbortSignal): Promise<SidebarResult<GitCommitDetailResult>> {
    return this.withProbe(request.path, signal, async () => {
      const [messageRes, filesRes] = await Promise.all([
        ...
        this.runner.run(
          ['diff-tree', '--no-commit-id', '--name-status', '-r', '-M', '-m', '--first-parent', '--root', '-z', request.hash],
          request.path,
          signal,
        ),
      ])
      if (!messageRes.ok) return { ok: false, error: mapRunnerError(messageRes, request.path) }
      if (!filesRes.ok) return { ok: false, error: mapRunnerError(filesRes, request.path) }
      return {
        ok: true,
        value: {
   ...
   */
  async discard(request: GitDiscardRequest, signal?: AbortSignal): Promise<SidebarResult<null>> {
    if (signal?.aborted === true) return { ok: false, error: sidebarError('cancelled', 'cancelled') }
    ...
    const runs: Array<Promise<SidebarResult<null>>> = []
    if (tracked.length > 0) {
    ...
    if (runs.length === 0) return { ok: true, value: null }
    const results = await Promise.all(runs)
    const firstFail = results.find(r => !r.ok)
    if (firstFail !== undefined) return firstFail as SidebarResult<null>
   ...
   */
  async commit(request: GitCommitRequest, signal?: AbortSignal): Promise<SidebarResult<GitCommitResult>> {
    if (signal?.aborted === true) return { ok: false, error: sidebarError('cancelled', 'cancelled') }
    return this.withProbe(request.path, signal, async () => {
      if (request.files.length > 0) {
        const add = await this.runner.run(['add', '--', ...request.files], request.path, signal)
        if (!add.ok) return { ok: false, error: mapRunnerError(add, request.path) }
      }
      const message = request.message.trim()
      const commit = await this.runner.run(['commit', '-F', '-'], request.path, signal, message)
      if (!commit.ok) return { ok: false, error: mapRunnerError(commit, request.path) }
      const hashRes = await this.runner.run(['rev-parse', 'HEAD'], request.path, signal)
      if (!hashRes.ok) return { ok: false, error: mapRunnerError(hashRes, request.path) }
  ...
  private async withProbe<T>(
    ...
    body: (head: string | undefined) => Promise<SidebarResult<T>>,
  ): Promise<SidebarResult<T>> {
    const probe = await this.probe(cwd, signal)
    if ('ok' in probe) return body(probe.head)
    return { ok: false, error: probe }
  }

  /** stage/unstage run a single fixed git command; failures map to errors. */
  private async applyToFiles(request: GitStageRequest, args: string[], signal?: AbortSignal): Promise<SidebarResult<null>> {
    if (signal?.aborted === true) return { ok: false, error: sidebarError('cancelled', 'cancelled') }
    const res = await this.runner.run(args, request.path, signal)
    if (!res.ok) return { ok: false, error: mapRunnerError(res, request.path) }
    return { ok: true, value: null }
  ...
  private group(entries: GitStatusEntry[]): Omit<GitStatusResult, 'head' | 'truncated'> {
  ...
  private parseLog(stdout: Buffer): GitLogEntry[] {
    ...
    for (const record of text.split('\x1e')) {
      ...
      if (hash === undefined || shortHash === undefined || authorName === undefined
        || authorEmail === undefined || authoredAtISO === undefined || subject === undefined) continue
 ...
 */
export function parseNameStatus(stdout: Buffer): GitCommitFile[] {
  const text = stdout.toString('utf8')
  if (text.length === 0) return []
  ...
  while (i < records.length) {
    ...
    if (statusField === undefined || statusField.length === 0) { i += 1; continue }
    const letter = statusField[0] as GitFileStatus
    if (letter === 'R' || letter === 'C') {
      const original = records[i + 1]
      const target = records[i + 2]
      if (original === undefined || original === '' || target === undefined || target === '') break
      const file: GitCommitFile = { status: letter, path: target, originalPath: original }
      const score = Number(statusField.slice(1))
      if (!Number.isNaN(score)) file.score = score
      files.push(file)
      i += 3
    } else {
      const p = records[i + 1]
