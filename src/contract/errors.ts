...
export type SidebarResult<T> =
  | { ok: true; value: T }
  | { ok: false; error: SidebarError }

/** Narrow helper: build a typed error branch. */
export function sidebarError(code: SidebarError['code'], message: string): Extract<SidebarError, { code: typeof code }> {
  return { code, message } as Extract<SidebarError, { code: typeof code }>
}
